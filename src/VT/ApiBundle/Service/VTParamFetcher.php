...
/*
 * OUR OWN ARD IMPLEMENTATION OF FOSRestBundle PARAMFETCHER
...
namespace VT\ApiBundle\Service;
...
/* WE REPLACED Regex by VTRegex, to catch null & empty values */
 ...
 */
class VTParamFetcher implements ParamFetcherInterface
{
    private $paramReader;
    private $request;
    private $params;
        ...
        if (!is_scalar($param)) {
            if (!$nullable) {
                if ($strict) {
                    ...
                    /* ADDED, EMPTY ERROR. Missing & Invalid PARAMS isn't the same error in our API */
                    if ($problem === 'empty') {
